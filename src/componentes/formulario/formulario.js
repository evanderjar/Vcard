        ...
        return <div>
          ...
          : null } 
            ...
            
            {
                ...
                <div class="form-group">
                    <label style={style.titulos}>
                    Telefono (con codigo pais y sin 0):
                    <input style={{fontSize: "15px"}} class="form-control" type="number" name="telefono" value={telefono} onChange={(event)=>{SetTelefono(event.target.value)}} required/>
                    </label>
                </div>
  ...
  const handleNext = async () => {
    ...
    if (activeStep === 2){
      ...
      if(actualizar){
        ...
        /***** MODIFICAR SI ESTA COLOCANDO EL NOMBRE DE USUARIO FACEBOOK****** */
        const split_facebook = facebook.split("/")
        var cuenta_facebook = facebook
        if(split_facebook.length === 1 && split_facebook[0] !== ""){
          console.log(cuenta_facebook)
          cuenta_facebook = "https://www.facebook.com/"+facebook
        }

        /***** MODIFICAR SI ESTA COLOCANDO EL NOMBRE DE USUARIO LINKEDIN****** */
        const split_linkedin = linkedin.split("/")
        var cuenta_linkedin = linkedin
        if(split_linkedin.length === 1 && split_linkedin[0] !== ""){
          console.log(cuenta_linkedin)
          cuenta_linkedin = "https://www.linkedin.com/in/"+linkedin
        }
          ...
          .then(snapshot => {
            ...
            if(contador === 1){
              ...
              db.collection("Datos_usuarios").doc(key).set({
                ...
                facebook:cuenta_facebook,
                linkedin:cuenta_linkedin,
                skype,
                web,
                foto_perfil:imagen,
        ...
        {activeStep === steps.length && (
          <Paper square elevation={0} className={classes.resetContainer}>
            {/* { existeCodigo ? 
              ...
              { existeCodigo ? 
                null
              : <span>Su link es: <a href={`http://tienda.deproinf.com.ve/#/${nombre_ruta}`}>{`http://tienda.deproinf.com.ve/#/${nombre_ruta}`} </a></span>}     
              <div></div>     
